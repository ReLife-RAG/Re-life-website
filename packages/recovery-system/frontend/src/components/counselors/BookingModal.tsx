...
export default function BookingModal({
  ...
  return (
    ...
    >
      <div className="relative flex flex-col md:flex-row rounded-3xl overflow-hidden shadow-2xl w-full max-w-3xl min-h-[430px] border border-[#DDE9E8]">
        <div className="flex flex-col items-center justify-center px-6 py-8 md:py-10 gap-5 w-full md:w-[250px] bg-gradient-to-br from-[#4A7C7C] to-[#3a6060]">
          <button
            type="button"
            aria-label="Close"
            onClick={onClose}
        ...
        <div className="flex-1 bg-white px-6 md:px-8 py-8 flex flex-col">
          <h2 className="text-lg md:text-xl font-semibold text-[#0f2420] mb-4">Book a Session</h2>
          ...
          {loadingSlots ? (
            <p className="text-sm text-[#6b8a87] mb-5">Loading slots...</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-5 max-h-44 overflow-auto pr-1">
              {slots.length === 0 ? (
                <p className="text-sm text-[#6b8a87]">No available slots right now.</p>
              ) : (
                slots.map((slot) => (
                  <button
                    ...
                    className={`py-2 px-2 rounded-lg text-sm font-medium border transition-all ${
                      !slot.available
                        ...
                        ? "bg-[#EBF4F4] border-[#4A7C7C] text-[#4A7C7C] shadow-sm"
                        : "border-[#DDE9E8] text-[#2d4a47] hover:border-[#4A7C7C] hover:text-[#4A7C7C]"
                    }`}
                  >
                    {slot.time}
                  </button>
          ...
          <input
            ...
            className="mb-3 w-full rounded-xl border border-[#DDE9E8] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#CFE1E1]"
          />
          ...
          <textarea
            ...
            rows={2}
            className="mb-4 w-full rounded-xl border border-[#DDE9E8] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#CFE1E1]"
          />

          <div className="bg-[#F4F9F8] rounded-xl p-4 mb-4 border border-[#DDE9E8]">
            <div className="flex justify-between text-sm text-[#2d4a47] mb-2">
              <span>Session Fee</span>
              <span>${counselor.fee}</span>
            </div>
            <div className="flex justify-between text-sm text-[#2d4a47] mb-3">
              <span>Service Tax</span>
              <span>${tax.toFixed(2)}</span>
            </div>
            <div className="flex justify-between font-semibold text-[#0f2420] border-t border-[#DDE9E8] pt-3">
              <span>Total</span>
              <span className="text-[#4A7C7C]">${total.toFixed(0)}</span>
            </div>
          </div>
          ...
          <button
            ...
            className={`w-full py-3 rounded-full font-semibold text-sm transition-all ${
              selected && !bookingLoading
                ? "bg-gradient-to-r from-[#4A7C7C] to-[#86D293] hover:opacity-95 text-white shadow-md"
